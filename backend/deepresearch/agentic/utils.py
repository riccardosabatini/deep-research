...
# from tavily import AsyncTavilyClient # Removed direct Tavily usage
...
async def web_search(
    ...
    """Fetch and summarize search results using the configured search provider.
    ...
    """
    # Step 1: Initialize SearchTools
    search_tools = SearchTools()
    
    # Step 2: Execute search queries asynchronously
    async def perform_single_search(query):
        try:
            return await search_tools.perform_search(query, max_results=max_results)
        except Exception as e:
            logging.error(f"Error performing search for query '{query}': {e}")
            return {"sources": [], "images": []}

    search_tasks = [perform_single_search(query) for query in queries]
    search_results_list = await asyncio.gather(*search_tasks)
    
    # Step 3: Deduplicate results by URL
    unique_results = {}
    for i, result_data in enumerate(search_results_list):
        query = queries[i]
        for source in result_data.get("sources", []):
            if source.url not in unique_results:
                unique_results[source.url] = {
                    "title": source.title,
                    "url": source.url,
    ...
    summarization_model = get_llm(config, model_name=configurable.task_model)
...
async def get_search_tool(search_api: SearchAPI):
    ...
    if search_api == SearchAPI.ANTHROPIC:
        ...
        # Configure Tavily search tool (via SearchTools)
        return [web_search]
